...
-- DM_UIHelperFunctions.lua
-- Reusable ImGui UI helpers for DM ReaperToolkit.
-- Requires Colors (global) to be set before use (via DM_ToolkitFunctionsLibrary.lua).
...
function UI.ImageButton(ctx, id, img, w, h, opts)
    ...
    if opts.three_state then
        -- Use InvisibleButton so we can query state before drawing the image.
